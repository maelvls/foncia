...
func coownersEndpoint(client *http.Client, uuid string) http.HandlerFunc {
	return logRequest(func(w http.ResponseWriter, r *http.Request) {
		...
		coowners, err := api.GetCouncilCoowners(client, uuid)
		if err != nil {
			logutil.Errorf("while getting coowners: %v", err)
			http.Error(w, "while getting coowners: "+err.Error(), http.StatusInternalServerError)
			return
		}
		...
		tmpl := template.Must(template.New("coowners").Parse(`
<!DOCTYPE html>
<html>
<head>
<title>Liste des co-propriétaires</title>
<meta charset="utf-8">
</head>
<body>
...
`))
		...
		for _, coowner := range coowners {
			...
			for _, lot := range coowner.Units {
				...
				for _, m := range mapping {
					...
					if strings.HasPrefix(m.Type, "T") {
						apartment = m.Description
					} else if strings.HasPrefix(m.Type, "Parking") {
			...
			coownersData = append(coownersData, tmplData{
		...
		err = tmpl.Execute(w, coownersData)
		if err != nil {
			logutil.Errorf("executing template: %v", err)
			http.Error(w, "while executing template: "+err.Error(), http.StatusInternalServerError)
			return
		}
	})
}
